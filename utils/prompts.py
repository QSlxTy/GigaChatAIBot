generate_text_prompt = (
    ...
    Длина каждой истории должна составлять от 3 до 6 предложений. Учитывай как можно больше контекста из ответов, 
    ...
    Ты должен отвечать строго по этому шаблону, без использования дополнительных символов или форматов. Дай ответ в формате list python.
    """
)
generate_photo_prompt = (
    ...
    Например, если все истории связаны одним героем, он должен быть описан одинаково, а сцены должны соответствовать логике развития событий.

    Используй до 15 слов для описания, пиши строго на английском языке.
    ...
    Пример вывода:
    ["A young hero on a snowy mountain, determined", 
     "A glowing sunrise over a bustling city, hopeful", 
     "A mysterious figure in a dark forest, suspenseful", 
     "A vibrant festival with colorful lights, joyful", 
     "A peaceful beach at sunset, serene"]

    Ты должен отвечать только по этому шаблону, строго следуя структуре, без дополнительных форматов или символов. 
    Дай ответ в формате list python.
    """
)
